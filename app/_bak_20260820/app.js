(function(w){
  ...
  var state = {
    classId: TW.store.read('current_class', savedCtx.classId || classes[0].id),
    ...
    hwTrackWeek: TW.store.read('hw_track_week', '第1周')
  };
  if (!classes.some(function(c){ return c.id === state.classId; })) state.classId = classes[0].id;
  ...
  function init(){
    renderClassPicker();
    var seen = false; try { seen = sessionStorage.getItem('wb_picker_seen') === '1'; } catch(e){}
    if (seen) { TW.$('#subjectPicker').classList.add('hidden'); TW.$('#app').classList.remove('hidden'); }
    bindGlobal();
    renderShell();
  ...
  function renderClassPicker(){
    var grid = TW.$('#subjectGrid');
    var cards = classes.map(function(c){
      return '<button class="subject-card ' + (c.id === state.classId ? 'current' : '') + '" data-class="' + c.id + '"><strong>' + TW.escape(c.name) + '</strong><small>所带班级 · 点击进入</small></button>';
    }).join('');
    grid.innerHTML = cards + '<button class="subject-card add" data-action="add-class-picker"><strong>＋ 新增班级</strong><small>创建新的班级</small></button>';
    TW.$$('[data-class]', grid).forEach(function(b){ b.onclick = function(){
      state.classId = b.dataset.class; state.area = 'teaching'; state.module = 'dashboard';
      TW.store.write('current_class', state.classId); TW.store.write('entered', true);
      try { sessionStorage.setItem('wb_picker_seen', '1'); } catch(e){}
      hydrate(); persistContext();
      TW.$('#subjectPicker').classList.add('hidden'); TW.$('#app').classList.remove('hidden'); renderShell();
    }; });
    TW.$$('[data-action="add-class-picker"]', grid).forEach(function(b){ b.onclick = function(){ addClassFromPicker(); }; });
  }

  function addClassFromPicker(){
    var name = prompt('请输入新班级名称（如：高一(7)班）');
    if (!name || !name.trim()) return;
    var cls = { id: 'c' + Date.now().toString(36), name: name.trim() };
    classes.push(cls); TW.store.write('classes', classes);
    state.classId = cls.id; state.area = 'teaching'; state.module = 'dashboard';
    TW.store.write('current_class', cls.id); TW.store.write('entered', true);
    try { sessionStorage.setItem('wb_picker_seen', '1'); } catch(e){}
    hydrate(); persistContext();
    TW.$('#subjectPicker').classList.add('hidden'); TW.$('#app').classList.remove('hidden'); renderShell();
    TW.toast('已创建班级：' + cls.name);
  }
  ...
  function bindGlobal(){
    TW.$('#subjectSelect').onchange = function(){
      state.classId = this.value; state.area = 'teaching'; state.module = 'dashboard';
      TW.store.write('current_class', state.classId); hydrate(); persistContext(); renderShell();
      TW.toast('已切换到 ' + classNameOf());
    };
    TW.$('#subjectPickerButton').onclick = manageClasses;
  ...
  function manageClasses(){
    var rows = classes.map(function(c){
      return '<div class="task-row"><div><strong>' + TW.escape(c.name) + '</strong><div class="note">' + (c.id === state.classId ? '当前班级' : '') + '</div></div><span><button class="button text" data-rename="' + c.id + '">改名</button><button class="button text danger-text" data-delclass="' + c.id + '">删除</button></span></div>';
    }).join('');
    TW.modal('班级管理', '<div class="note" style="margin-bottom:10px">管理所带班级：新增、改名或删除。切换班级请在左上角下拉框操作。</div>' + rows, '<button class="button secondary" data-close>关闭</button><button class="button" id="addClassBtn">+ 新增班级</button>');
    TW.$('#addClassBtn').onclick = function(){ TW.$('#modalRoot').innerHTML = ''; addClassFromPicker(); };
    TW.$$('[data-rename]', TW.$('#modalRoot')).forEach(function(b){ b.onclick = function(){
      var c = classes.find(function(x){ return x.id === b.dataset.rename; }); if (!c) return;
      var name = prompt('班级新名称：', c.name); if (!name || !name.trim()) return;
      c.name = name.trim(); TW.store.write('classes', classes); hydrate(); TW.$('#modalRoot').innerHTML = ''; renderShell(); TW.toast('已改名');
    }; });
    TW.$$('[data-delclass]', TW.$('#modalRoot')).forEach(function(b){ b.onclick = function(){
      if (classes.length <= 1) { TW.toast('至少保留一个班级', 'danger'); return; }
      if (!confirm('确认删除该班级及其全部数据？')) return;
      var id = b.dataset.delclass;
      classes = classes.filter(function(x){ return x.id !== id; });
      TW.store.write('classes', classes);
      ['students','attendance','exams','subjects','homework','meetings','meeting_plan','family','parent_meetings','family_plan','dorm','dorm_visits','mental','quality','selection','affairs','tasks','cleaning','points','talks'].forEach(function(k){ TW.store.remove(k + '_' + id); });
      if (state.classId === id) { state.classId = classes[0].id; TW.store.write('current_class', state.classId); }
      hydrate(); TW.$('#modalRoot').innerHTML = ''; renderShell(); TW.toast('已删除班级');
    }; });
  }
  ...
  function renderShell(){
    ...
    TW.$('#subjectSelect').innerHTML = classes.map(function(c){ return '<option value="' + c.id + '" ' + (c.id === state.classId ? 'selected' : '') + '>' + TW.escape(c.name) + '</option>'; }).join('');
  ...
  function campusHero(){
    ...
    var urgentHtml = urgent.length
      ? '<div class="hero-tasks-head">近 3 天任务提醒 <span class="hero-tasks-count">' + urgent.length + ' 项</span></div><ul class="hero-tasks-list">' + urgent.slice(0, 4).map(function(t){ return '<li role="button" tabindex="0" class="quick-entry" data-action="goto-task" data-kind="' + TW.escape(t.kind || '') + '" title="前往对应板块">' + (taskTarget(t.kind) !== 'notice' ? '<span class="hero-task-title">' + TW.escape(t.title) + '</span>' : '<span class="hero-task-title">' + TW.escape(t.title) + '</span>') + '<span class="hero-task-due">' + TW.escape(t.due) + ' ›</span></li>'; }).join('') + (urgent.length > 4 ? '<li class="hero-tasks-more">… 还有 ' + (urgent.length - 4) + ' 项待办</li>' : '') + '</ul>'
      : '<div class="hero-tasks-empty">近 3 天无待办任务，一切就绪</div>';
    return '<section class="campus-hero"><div class="campus-visual"><div class="campus-backdrop"></div><div class="campus-grid-lines"></div><div class="campus-hero-copy"><span class="eyebrow">HOMEROOM OPERATIONS</span><h2>' + classNameOf() + '班级运行总览</h2>' + urgentHtml + '</div></div><div class="campus-metrics"><div><span>今日异常</span><strong>' + TW.attendance.length + '</strong><small>考勤异常记录</small></div><div><span>班级人数</span><strong>' + TW.students.length + '</strong><small>' + classNameOf() + '</small></div><div><span>待办闭环</span><strong>' + pending.length + '</strong><small>未处理</small></div><div class="campus-signal"><i></i><span>本地磁盘</span><strong>已保存</strong></div></div></section>';
  }

  /* ================= 页面构建器 ================= */
  function renderModule(){
    var f = { dashboard: dashboard, affairs: affairs, students: students, attendance: attendance, scores: scores, homework: homework, meeting: meeting, family: family, dorm: dorm, quality: quality, selection: selection, cleaning: cleaning, points: points, alerts: alerts, personal: personal };
  ...
  /* ================= 各班作业跟踪（周作业模板模块） ================= */
  /* 数据：TW.weeklyHomework = [{ cls, head, subject, rep, week, days:{周一..周五} }]（跨班级全局存储）
   * 上传入口：适配 templates/27-周作业跟踪.csv（班级,班主任,班主任学科,政治课代表,第X周,周一~周五）
   * 也兼容原「高一各班记录跟踪」xlsx 布局（行1=周次表头，行2=日期，行3起=每班一行） */
  var WEEK_DAY_KEYS = ['周一', '周二', '周三', '周四', '周五'];
  function weeklyTrackRows(week){
    return (TW.weeklyHomework || []).filter(function(r){ return r.week === week; });
  }
  function weeklyClassNames(){
    var seen = {}, out = [];
    (TW.weeklyHomework || []).forEach(function(r){ if (r.cls && !seen[r.cls]) { seen[r.cls] = 1; out.push(r.cls); } });
    return out;
  }
  function weeklySave(){ gw('weekly_homework', TW.weeklyHomework); }
  function weeklyTrackSection(){
    var week = state.hwTrackWeek || '第1周';
    var rows = weeklyTrackRows(week);
    var dayNames = WEEK_DAY_KEYS;
    var headCells = ['<th>班级</th><th>班主任</th><th>班主任学科</th><th>政治课代表</th>']
      .concat(dayNames.map(function(d){ return '<th>' + d + '</th>'; }))
      .concat('<th>操作</th>');
    var body = rows.length ? rows.map(function(r, i){
      var cells = ['<td class="weekly-cell" data-action="weekly-edit-cls" data-widx="' + i + '" title="点击编辑班级信息"><strong>' + TW.escape(r.cls || '') + '</strong></td>'
        , '<td class="weekly-cell" data-action="weekly-edit-cls" data-widx="' + i + '">' + TW.escape(r.head || '') + '</td>'
        , '<td class="weekly-cell" data-action="weekly-edit-cls" data-widx="' + i + '">' + TW.escape(r.subject || '') + '</td>'
        , '<td class="weekly-cell" data-action="weekly-edit-cls" data-widx="' + i + '">' + TW.escape(r.rep || '') + '</td>'];
      dayNames.forEach(function(d){
        var v = (r.days && r.days[d]) || '';
        cells.push('<td class="weekly-cell" data-action="weekly-edit-day" data-widx="' + i + '" data-day="' + d + '" title="点击编辑' + d + '作业">' + (v ? TW.escape(v.length > 30 ? v.slice(0, 30) + '…' : v) : '<span class="hw-empty">＋ 录入</span>') + '</td>');
      });
      cells.push('<td><button class="button text danger-text" data-action="weekly-del-row" data-widx="' + i + '">删除</button></td>');
      return '<tr>' + cells.join('') + '</tr>';
    }).join('') : '<tr><td colspan="10" style="text-align:center;color:var(--ink-3);padding:18px">本周还没有记录 — 点击「上传周作业模板」导入，或「＋ 添加班级」手动录入</td></tr>';
    return '<section class="section"><div class="section-title">各班作业跟踪 <small>' + week + ' · 适配「27-周作业跟踪」模板上传（支持 xlsx / csv），ABCD 列可点击编辑</small></div><div class="section-body"><div class="toolbar" style="margin-bottom:12px;flex-wrap:wrap;gap:8px"><button class="button secondary" data-action="weekly-week-prev">‹ 上周</button><select id="weeklyWeekSelect" class="weekly-week-select" aria-label="选择周次">' + weeklyWeekOptions(week) + '</select><button class="button secondary" data-action="weekly-week-next">下周 ›</button><button class="button" data-action="weekly-upload">上传周作业模板</button><button class="button secondary" data-action="weekly-add-cls">＋ 添加班级</button><button class="button secondary" data-action="weekly-template-download">⬇ 下载模板</button></div><div class="data-table-wrap"><table class="data-table no-edit-table weekly-table"><thead><tr>' + headCells.join('') + '</tr></thead><tbody>' + body + '</tbody></table></div></div></section>';
  }
  function weeklyWeekOptions(current){
    var opts = [];
    for (var i = 1; i <= 24; i++) opts.push('第' + i + '周');
    return opts.map(function(w){ return '<option' + (w === current ? ' selected' : '') + '>' + w + '</option>'; }).join('');
  }
  function weeklySetWeek(week){
    state.hwTrackWeek = week;
    TW.store.write('hw_track_week', week);
    renderShell();
  }
  ...
  function bindModule(){
    ...
    if (state.module === 'homework') {
      ...
      var wws = TW.$('#weeklyWeekSelect', TW.$('#mainContent'));
      if (wws) wws.onchange = function(){ weeklySetWeek(this.value); };
    }
  ...
  function action(a, el){
    var f = {
      ...
      'weekly-week-prev': function(){ var m = String(state.hwTrackWeek || '第1周').match(/第(\d+)周/); var n = m ? Math.max(1, Number(m[1]) - 1) : 1; weeklySetWeek('第' + n + '周'); },
      'weekly-week-next': function(){ var m = String(state.hwTrackWeek || '第1周').match(/第(\d+)周/); var n = m ? Number(m[1]) + 1 : 2; weeklySetWeek('第' + n + '周'); },
      'weekly-upload': weeklyUpload,
      'weekly-add-cls': weeklyAddClass,
      'weekly-edit-cls': function(){ weeklyEditClass(Number(el.dataset.widx)); },
      'weekly-edit-day': function(){ weeklyEditDay(Number(el.dataset.widx), el.dataset.day); },
      'weekly-del-row': function(){ weeklyDeleteRow(Number(el.dataset.widx)); },
      'weekly-template-download': weeklyTemplateDownload,
  ...
  /* ============ 各班作业跟踪（周作业模板模块） ============ */
  function weeklyWeekNum(week){ var m = String(week || '').match(/第(\d+)周/); return m ? Number(m[1]) : 0; }
  function weeklySortRows(rows){
    return rows.slice().sort(function(a, b){
      var wn = weeklyWeekNum(a.week) - weeklyWeekNum(b.week);
      if (wn !== 0) return wn;
      return String(a.cls || '').localeCompare(String(b.cls || ''), 'zh');
    });
  }
  function weeklyEnsureRow(cls, head, subject, rep, week){
    var list = TW.weeklyHomework;
    for (var i = 0; i < list.length; i++){
      if (list[i].cls === cls && list[i].week === (week || state.hwTrackWeek)) return list[i];
    }
    var rec = { cls: cls, head: head || '', subject: subject || '', rep: rep || '', week: week || state.hwTrackWeek, days: { 周一: '', 周二: '', 周三: '', 周四: '', 周五: '' } };
    list.push(rec);
    return rec;
  }
  function weeklyTemplateDownload(){
    var headers = ['班级', '班主任', '班主任学科', '政治课代表', '第X周', '周一', '周二', '周三', '周四', '周五'];
    var example = ['高一16班', '李林华', '数学', '肖玥希、谭欢', '第1周', '开学第一课：完成导学案填空', '预习5分钟', '1.1.1从原始社会到奴隶社会', '', ''];
    downloadCSV('27-周作业跟踪模板.csv', [headers, example]);
    TW.toast('模板已下载（含表头与示例行，可直接编辑后上传）');
  }
  function weeklyAddClass(){
    TW.modal('添加班级（' + state.hwTrackWeek + '）', '<div class="alert info">ABCD 列为本模块内用户自行输入的内容：班级、班主任、班主任学科（有助于理解该班学生偏好）、政治课代表。</div><div class="form-grid"><div class="field"><label class="required">班级</label><input id="wkCls" placeholder="如：高一16班"></div><div class="field"><label>班主任</label><input id="wkHead" placeholder="如：李林华"></div><div class="field"><label>班主任学科</label><input id="wkSubject" placeholder="如：数学"></div><div class="field"><label>政治课代表</label><input id="wkRep" placeholder="如：肖玥希、谭欢"></div></div>', '<button class="button secondary" data-close>取消</button><button class="button" id="wkSave">保存</button>');
    setTimeout(function(){ var i = TW.$('#wkCls'); if (i) i.focus(); }, 50);
    TW.$('#wkSave').onclick = function(){
      var cls = TW.$('#wkCls').value.trim();
      if (!cls) { TW.toast('请填写班级', 'danger'); return; }
      var existing = (TW.weeklyHomework || []).filter(function(r){ return r.cls === cls && r.week === state.hwTrackWeek; });
      if (existing.length) { TW.toast('该班级本周已存在，可直接点击单元格编辑', 'danger'); return; }
      weeklyEnsureRow(cls, TW.$('#wkHead').value.trim(), TW.$('#wkSubject').value.trim(), TW.$('#wkRep').value.trim(), state.hwTrackWeek);
      weeklySave();
      TW.$('#modalRoot').innerHTML = ''; renderShell();
      logRecord('各班作业跟踪：添加班级', cls + ' ' + state.hwTrackWeek);
      TW.toast('已添加 ' + cls);
    };
  }
  function weeklyEditClass(widx){
    var rows = weeklyTrackRows(state.hwTrackWeek);
    var r = rows[widx];
    if (!r) return;
    TW.modal('编辑班级信息 · ' + r.cls, '<div class="alert info">ABCD 列为本模块内用户自行输入的内容。</div><div class="form-grid"><div class="field"><label class="required">班级</label><input id="wkCls" value="' + TW.escape(r.cls || '') + '"></div><div class="field"><label>班主任</label><input id="wkHead" value="' + TW.escape(r.head || '') + '"></div><div class="field"><label>班主任学科</label><input id="wkSubject" value="' + TW.escape(r.subject || '') + '"></div><div class="field"><label>政治课代表</label><input id="wkRep" value="' + TW.escape(r.rep || '') + '"></div></div>', '<button class="button secondary" data-close>取消</button><button class="button" id="wkSave">保存</button>');
    TW.$('#wkSave').onclick = function(){
      var cls = TW.$('#wkCls').value.trim();
      if (!cls) { TW.toast('请填写班级', 'danger'); return; }
      var dup = (TW.weeklyHomework || []).some(function(x){ return x !== r && x.cls === cls && x.week === r.week; });
      if (dup) { TW.toast('该班级本周已存在', 'danger'); return; }
      r.cls = cls; r.head = TW.$('#wkHead').value.trim(); r.subject = TW.$('#wkSubject').value.trim(); r.rep = TW.$('#wkRep').value.trim();
      weeklySave();
      TW.$('#modalRoot').innerHTML = ''; renderShell();
      logRecord('各班作业跟踪：更新班级信息', cls);
      TW.toast('已保存班级信息');
    };
  }
  function weeklyEditDay(widx, day){
    var rows = weeklyTrackRows(state.hwTrackWeek);
    var r = rows[widx];
    if (!r) return;
    var val = (r.days && r.days[day]) || '';
    TW.modal('编辑 ' + day + ' 作业 · ' + r.cls, '<div class="alert info">' + day + '（' + state.hwTrackWeek + '）的作业内容，可多行。</div><div class="form-grid"><div class="field full"><label class="required">' + day + ' 作业</label><textarea id="wkDay" style="min-height:140px" placeholder="如：完成导学案填空、背诵、预习…">' + TW.escape(val) + '</textarea></div></div>', '<button class="button secondary" data-close>取消</button><button class="button" id="wkSave">保存</button>');
    setTimeout(function(){ var i = TW.$('#wkDay'); if (i) { i.focus(); i.selectionStart = i.selectionEnd = i.value.length; } }, 50);
    TW.$('#wkSave').onclick = function(){
      if (!r.days) r.days = { 周一: '', 周二: '', 周三: '', 周四: '', 周五: '' };
      r.days[day] = TW.$('#wkDay').value.trim();
      weeklySave();
      TW.$('#modalRoot').innerHTML = ''; renderShell();
      logRecord('各班作业跟踪：录入作业', r.cls + ' ' + day);
      TW.toast('已保存 ' + r.cls + ' ' + day);
    };
  }
  function weeklyDeleteRow(widx){
    var rows = weeklyTrackRows(state.hwTrackWeek);
    var r = rows[widx];
    if (!r) return;
    if (!confirm('确认删除「' + r.cls + ' ' + state.hwTrackWeek + '」的记录吗？')) return;
    var i = TW.weeklyHomework.indexOf(r);
    if (i >= 0) TW.weeklyHomework.splice(i, 1);
    weeklySave(); renderShell(); TW.toast('已删除');
  }
  function weeklyUpload(){
    var m = TW.modal('上传周作业模板（' + state.hwTrackWeek + '）', '<div class="import-dialog"><div class="alert info">支持 <strong>xlsx / csv</strong>：简化模板表头「班级,班主任,班主任学科,政治课代表,第X周,周一~周五」（可先点「下载模板」）；也兼容原「高一各班记录跟踪」xlsx 布局。上传后先预览，确认才写入。</div><div class="toolbar" style="margin:10px 0"><button class="button secondary" id="weeklyTemplateBtn">⬇ 下载模板</button></div><div class="field"><label class="required">选择模板文件</label><input id="weeklyFile" type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"></div><div id="weeklyPreview" class="import-preview note">尚未选择文件</div></div>', '<button class="button secondary" data-close>取消</button><button class="button" id="weeklyApply" disabled>确认导入</button>');
    var modalEl = m.root.querySelector('.modal'); if (modalEl) modalEl.classList.add('import-modal');
    var input = TW.$('#weeklyFile'), preview = TW.$('#weeklyPreview'), apply = TW.$('#weeklyApply');
    var parsed = null;
    TW.$('#weeklyTemplateBtn').onclick = weeklyTemplateDownload;
    input.onchange = function(){
      var file = input.files && input.files[0]; if (!file) return;
      if (file.size > 10 * 1024 * 1024) { preview.textContent = '文件超过 10MB，已拒绝。'; return; }
      ...
      reader.onload = function(){
        try {
          ...
          fetch('/api/parse-weekly', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            ...
            .then(function(r){
              if (r.status !== 200 || !r.json.ok) { parsed = null; apply.disabled = true; preview.textContent = '解析失败：' + (r.json && r.json.error || '未知错误'); return; }
              parsed = r.json.rows;
              var formatNote = r.json.format === 'original' ? '（原版 excel 布局，按周拆分）' : '（简化模板）';
              var byWeek = {};
              parsed.forEach(function(x){ byWeek[x.week || ''] = (byWeek[x.week || ''] || 0) + 1; });
              var weeksNote = Object.keys(byWeek).map(function(k){ return k + '×' + byWeek[k]; }).join('，');
              preview.innerHTML = '<strong>校验通过：' + parsed.length + ' 条' + formatNote + '</strong><div class="note">涉及周次：' + (weeksNote || '—') + '<br>导入后按「班级 + 周次」匹配：已存在的班级周记录会合并作业，新记录自动追加。</div><div class="import-preview-grid">' + parsed.slice(0, 5).map(function(x){ return '<span>' + TW.escape(x.cls + ' · ' + (x.week || '未标周')) + '：' + WEEK_DAY_KEYS.filter(function(d){ return x.days && x.days[d]; }).map(function(d){ return d + ' 已填'; }).join('，') + '</span>'; }).join('') + '</div>';
              apply.disabled = false;
            })
            .catch(function(err){ parsed = null; apply.disabled = true; preview.textContent = '请求失败：' + (err && err.message || err); });
        } catch (error) { parsed = null; apply.disabled = true; preview.textContent = '读取失败：' + error.message; }
      };
      reader.onerror = function(){ preview.textContent = '文件读取失败，请重新选择。'; };
    ...
    apply.onclick = function(){
      ...
      parsed.forEach(function(x){
        var week = x.week || state.hwTrackWeek;
        var rec = null;
        (TW.weeklyHomework || []).forEach(function(r){ if (!rec && r.cls === x.cls && r.week === week) rec = r; });
        if (rec) { updated++; WEEK_DAY_KEYS.forEach(function(d){ if (x.days && x.days[d]) { if (!rec.days) rec.days = {}; rec.days[d] = x.days[d]; } }); }
        else { added++; TW.weeklyHomework.push({ cls: x.cls, head: x.head || '', subject: x.subject || '', rep: x.rep || '', week: week, days: { 周一: x.days && x.days['周一'] || '', 周二: x.days && x.days['周二'] || '', 周三: x.days && x.days['周三'] || '', 周四: x.days && x.days['周四'] || '', 周五: x.days && x.days['周五'] || '' } }); }
      });
      weeklySave();
      TW.$('#modalRoot').innerHTML = ''; renderShell();
      logRecord('各班作业跟踪：导入周作业模板', '新增 ' + added + ' 条、更新 ' + updated + ' 条');
  ...
  function openProfile(){
    TW.modal('个人信息', '<div class="grid-3">' + [['当前班级', classNameOf()], ['身份', classNameOf() + '班主任'], ['学期', TW.semester]].map(function(p){ return '<div class="stat-card"><div class="stat-label">' + p[0] + '</div><div class="stat-value" style="font-size:22px">' + TW.escape(p[1]) + '</div></div>'; }).join('') + '</div><div style="max-height:58vh;overflow:auto;padding-right:6px;margin-top:4px"><div id="themeSection" style="margin-bottom:10px"></div><div id="lockSection" style="margin-bottom:10px"></div><div id="dataSection"></div></div>', '<button class="button secondary" data-close>关闭</button>');
