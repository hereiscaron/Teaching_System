...
const GLOBAL_TABLES = {
  ...
  lessons:   { key: 'lessons',        fields: [['班级','cls'],['日期','date'],['上课内容','content'],['上课计划','plan'],['应收作业','homework'],['备注','note']], special: 'lessons' }
};
...
function cmdImport(name, file, classId, mode) {
  ...
  withSnapshot(() => {
    ...
    if (def.special === 'lessons') {
      // lessons 是 [{id, cls, date, content, plan, homework, note, color}]；导入补 id 与 color
      next = next.map((o) => ({ id: 'ls_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 6), cls: o.cls, date: o.date, content: o.content || '', plan: o.plan || '', homework: o.homework || '', note: o.note || '', color: '' }));
    }
