(function(w){
  ...
  function cd(base, seed){
    ...
    if (v == null) return (state.classId === 'c1') ? seed : (Array.isArray(seed) ? [] : seed);
    return v;
  }
  ...
  function mirrorFromTable(m, rows){
    ...
    if (m === 'students') {
      return rows.map(function(r){
        return { name: String(r[0] == null ? '' : r[0]), id: String(r[1] == null ? '' : r[1]), gender: String(r[2] == null ? '' : r[2]), status: String(r[3] == null ? '' : r[3]), dorm: String(r[4] == null ? '' : r[4]), parent: String(r[5] == null ? '' : r[5]), phone: String(r[6] == null ? '' : r[6]), focusMark: String(r[7] == null ? '' : r[7]), focus: /关注/.test(String(r[7] || '')) ? 2 : 0, focusReason: '' };
      });
    }
    if (m === 'selection') {
  ...
  function renderShell(){
    ...
    var mods = state.area === 'teaching' ? TW.teachingModules : TW.scoreModules;
    TW.$('#sidebar').innerHTML = '<div class="sidebar-group-title">' + (state.area === 'teaching' ? '班级工作' : '成绩管理') + '</div>' + mods.map(function(m){ return '<button class="sidebar-item ' + (state.module === m[0] ? 'active' : '') + '" data-module="' + m[0] + '"><span class="sidebar-icon">' + TW.svgIcon(m[2]) + '</span><span>' + m[1] + '</span></button>'; }).join('') + '<div class="sidebar-divider"></div><button class="sidebar-item" id="profileNav" title="个人信息：锁屏密码与本机数据"><span class="sidebar-icon">' + TW.svgIcon('gear') + '</span><span>个人信息</span></button>';
  ...
  function affairs(){
    ...
    for (var d = 1; d <= days; d++){
      ...
      cells += '<div class="cal-cell cal-clickable ' + (today ? 'today' : '') + '" data-date="' + ds + '" tabindex="0" role="button" aria-label="' + ds + '，点击查看或录入当日事务"><div class="cal-day">' + d + '</div>' + evs.map(function(e){ return '<div class="cal-event ' + kindClass(e.kind) + '">' + TW.escape(e.title) + '</div>'; }).join('') + '</div>';
    }
  ...
  function personal(){
    ...
    for (var d = 1; d <= days; d++){
      ...
      var evs = byDate[ds] || [];
      var today = (new Date().getFullYear() === y && new Date().getMonth() === m && new Date().getDate() === d);
      cells += '<div class="cal-cell cal-clickable ' + (today ? 'today' : '') + '" data-date="' + ds + '" data-pcal="1" tabindex="0" role="button" aria-label="' + ds + '，点击查看或新增个人备忘"><div class="cal-day">' + d + '</div>' + evs.map(function(e){ return '<div class="cal-event c-personal">' + TW.escape(e.title) + '</div>'; }).join('') + '</div>';
    }
    var monthLabel = y + '年' + (m + 1) + '月';
  ...
  function quickAddPersonal(date){
    ...
    var listHtml = list.length
      ? '<ul class="mini-list" style="max-height:30vh;overflow:auto">' + list.map(function(x){ return '<li><span>' + TW.escape(x.title) + '</span><span><button class="button text danger-text" data-memo-id="' + x.id + '">删除</button></span>' + (x.note ? '<div class="note">' + TW.escape(x.note) + '</div>' : '') + '</li>'; }).join('') + '</ul>'
      : '<div class="alert info">当日暂无备忘，可直接录入</div>';
    TW.modal(date + ' 个人备忘', '<div class="section-title" style="margin-bottom:10px">当日备忘</div>' + listHtml + '<div class="section-title" style="margin:18px 0 10px">快速录入</div><div class="form-grid"><div class="field"><label class="required">日期</label><input id="pmDate" type="date" value="' + date + '" disabled style="background:var(--table-head)"></div><div class="field full"><label class="required">事项</label><input id="pmTitle" placeholder="如：备课、家长会、提交材料"></div><div class="field full"><label>备注</label><input id="pmNote" placeholder="补充说明（可选）"></div></div>', '<button class="button secondary" data-close>关闭</button><button class="button" id="pmSave">保存到该日</button>');
    setTimeout(function(){ var i = TW.$('#pmTitle'); if (i) i.focus(); }, 50);
    TW.$$('[data-memo-id]', TW.$('#modalRoot')).forEach(function(b){ b.onclick = function(){ if (!confirm('确认删除该备忘吗？')) return; TW.personalMemo = TW.personalMemo.filter(function(x){ return x.id !== b.dataset.memoId; }); gw('personal_memo', TW.personalMemo); TW.$('#modalRoot').innerHTML = ''; renderShell(); TW.toast('已删除'); }; });
    TW.$('#pmSave').onclick = function(){ var t = TW.$('#pmTitle').value.trim(); if (!t) { TW.toast('请填写事项', 'danger'); return; } TW.personalMemo.push({ id: uid('pm'), date: date, title: t, note: TW.$('#pmNote').value.trim() }); gw('personal_memo', TW.personalMemo); TW.$('#modalRoot').innerHTML = ''; renderShell(); logRecord('个人事务：新增备忘', date + ' ' + t); TW.toast('已添加到 ' + date); };
  }
  function newCleaning(){
    var weekOpts = CLEAN_WEEKS.map(function(w){ return '<option' + (w === todayLabel() ? ' selected' : '') + '>' + w + '</option>'; }).join('');
    var memberOpts = (TW.students || []).map(function(s){ return '<option value="' + TW.escape(s.name) + '">' + TW.escape(s.name) + '</option>'; }).join('');
    TW.modal('新增值日排班', '<div class="form-grid"><div class="field"><label class="required">星期</label><select id="clWeek">' + weekOpts + '</select></div><div class="field full"><label class="required">值日学生</label><div class="member-picker"><input id="clMembers" placeholder="如：张雨桐、李明轩" autocomplete="off"><div class="member-chips" id="clChips"></div></div><small class="note">从下方名单点选学生（可多选），或直接输入姓名，用顿号分隔</small><div class="member-options" id="clOptions">' + memberOpts + '</div></div><div class="field full"><label class="required">值日内容</label><input id="clTask" placeholder="如：扫地、拖地、擦黑板（可自定义）"><div class="task-quick" id="clQuick"></div></div></div>', '<button class="button secondary" data-close>取消</button><button class="button" id="clSave">保存排班</button>');
    // 常用值日内容快捷标签（点击填入）
    ...
    function refreshChips(){
      ...
      TW.$$('#clOptions option').forEach(function(o){
        o.selected = names.indexOf(o.value) >= 0;
        o.style.background = names.indexOf(o.value) >= 0 ? '#eef4ff' : '';
      });
    }
    ...
    TW.$('#clOptions').addEventListener('change', function(){ addMember(this.value); this.selectedIndex = 0; });
    refreshChips();
  ...
  function newStudent(){
    ...
    TW.$('#sSave').onclick = function(){ var name = TW.$('#sName').value.trim(); if (!name) { TW.toast('请填写姓名', 'danger'); return; }
      var focusMark = '';
      TW.students.push({ name: name, id: TW.$('#sId').value.trim(), gender: TW.$('#sGender').value, status: TW.$('#sStatus').value, dorm: TW.$('#sDorm').value.trim(), parent: TW.$('#sParent').value.trim(), phone: TW.$('#sPhone').value.trim(), focus: 0 });
  ...
  function viewStudent(name){
    ...
    var parts = [];
    // 基本信息
    parts.push('<section class="section"><div class="section-title">基本信息</div><div class="section-body"><div class="grid-2">' + [['学籍号', s.id || '—'], ['性别', s.gender || '—'], ['学籍状态', s.status || '—'], ['宿舍', s.dorm || '—'], ['家长', s.parent || '—'], ['联系电话', s.phone || '—'], ['重点关注', s.focus > 0 ? ('L' + s.focus + ' · ' + (s.focusReason || '')) : '无'], ['当前积分', pointTotal(sid) + ' 分']].map(function(p){ return '<div class="stat-card"><div class="stat-label">' + p[0] + '</div><div class="stat-value" style="font-size:15px">' + TW.escape(String(p[1])) + '</div></div>'; }).join('') + '</div></div></section>');
    // 分数明细（最近 8 条）
    var logs = pointLogs(sid);
  ...
  function newAffair(){
    TW.modal('新增班级事务', '<div class="form-grid"><div class="field"><label class="required">日期</label><input id="afDate" type="date" value="' + (state.calYear + '-' + String(state.calMonth + 1).padStart(2,'0') + '-01') + '"></div><div class="field"><label class="required">标题</label><input id="afTitle"></div><div class="field"><label>类型</label><select id="afKind"><option>德育</option><option>家校</option><option>活动</option><option>教学</option><option>考试</option></select></div><div class="field full"><label>备注</label><input id="afNote"></div></div>', '<button class="button secondary" data-close>取消</button><button class="button" id="afSave">保存</button>');
    TW.$('#afSave').onclick = function(){ var t = TW.$('#afTitle').value.trim(); if (!t) { TW.toast('请填写标题', 'danger'); return; } TW.affairs.push({ date: TW.$('#afDate').value, title: t, kind: TW.$('#afKind').value, note: TW.$('#afNote').value.trim() }); cw('affairs', TW.affairs); TW.$('#modalRoot').innerHTML = ''; renderShell(); logRecord('班级事务：新增事务', t); TW.toast('已新增事务'); };
  }
  /* 点击日历日期格：查看当日事务 + 快速录入该日工作（日期锁定） */
  function quickAddAffair(date){
    var list = (TW.affairs || []).filter(function(x){ return x.date === date; });
    var listHtml = list.length
      ? '<ul class="mini-list" style="max-height:30vh;overflow:auto">' + list.map(function(x){ return '<li><span>' + TW.escape(x.title) + '</span><span class="status-badge info">' + TW.escape(x.kind || '') + '</span>' + (x.note ? '<div class="note">' + TW.escape(x.note) + '</div>' : '') + '</li>'; }).join('') + '</ul>'
      : '<div class="alert info">当日暂无事务，可直接录入</div>';
    TW.modal(date + ' 班级事务', '<div class="section-title" style="margin-bottom:10px">当日事务</div>' + listHtml + '<div class="section-title" style="margin:18px 0 10px">快速录入</div><div class="form-grid"><div class="field"><label class="required">日期</label><input id="afDate" type="date" value="' + date + '" disabled style="background:var(--table-head)"></div><div class="field"><label class="required">标题</label><input id="afTitle" placeholder="如：主题班会、家长会、考试"></div><div class="field"><label>类型</label><select id="afKind"><option>德育</option><option>家校</option><option>活动</option><option>教学</option><option>考试</option></select></div><div class="field full"><label>备注</label><input id="afNote" placeholder="补充说明（可选）"></div></div>', '<button class="button secondary" data-close>关闭</button><button class="button" id="afSave">保存到该日</button>');
    setTimeout(function(){ var i = TW.$('#afTitle'); if (i) i.focus(); }, 50);
    TW.$('#afSave').onclick = function(){ var t = TW.$('#afTitle').value.trim(); if (!t) { TW.toast('请填写标题', 'danger'); return; } TW.affairs.push({ date: date, title: t, kind: TW.$('#afKind').value, note: TW.$('#afNote').value.trim() }); cw('affairs', TW.affairs); TW.$('#modalRoot').innerHTML = ''; renderShell(); logRecord('班级事务：新增事务', date + ' ' + t); TW.toast('已添加到 ' + date); };
  }
